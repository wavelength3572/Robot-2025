...
public class IntakeIOVirtualSim implements IntakeIO {
  ...
  private static final double MIN_FEED_TIME = 0.40; // Min delay before coral enters
  private static final double MAX_FEED_TIME = 0.9; // Max delay before coral enters
  ...
  private static final double FEEDER_THRESHOLD =
      .9; // Robot must be within 0.5 meters to receive coral
    ...
    if (currentIntakeState == IntakeState.PULL) {
      if (isNearCorrectFeeder() && RobotStatus.getCurrentPreset() == CoralSystemPresets.PICKUP) {
        if (!pullTimerStarted) {
          ...
          Logger.recordOutput("C0.o0ralSystem/FakeCoralIntakeDelay", randomPullDelay);
          pullTimer.reset();
          pullTimer.start();
